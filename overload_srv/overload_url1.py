...
def main(num_threads):
    ...
    for _ in range(num_threads):
        t = threading.Thread(target=perform_prediction, args=(url1, data_cam12, headers))
        t.start()
        threads.append(t)
    ...
    while not stop_signal:
        ...
        make_prediction(url1, data_cam12, headers)
        end_time = time.time()
        delay = end_time - start_time  # Calculate the elapsed time
        print(f"URL1 w/ {num_threads+1} procs |  Delay = {delay*1000} ms | ")  
