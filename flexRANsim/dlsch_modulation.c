...
// 1 --> activates , 0 --> deactivates my codes
#define PDSCH_OKU 1
                     ...
                     LTE_eNB_DLSCH_t *dlsch1)
{
  ...
  #if (PDSCH_OKU == 1)
    //****** PDSCH AMPLITUDES READING FROM .TXT FILE ****** //
    int elementsToRead = 25;
    double *newPDSCH_RB = (double*)malloc(elementsToRead * sizeof(double));
    int curr_id;
    if (frame_parms->N_RB_DL == 25){
      FILE* fd = fopen("/home/arled/Desktop/PDSCH.txt","r");
      if(fscanf(fd,"%d",&curr_id));
      if (curr_id == prev_id) printf("PDSCH is being generated. WAIT... \n");
      else{
        printf("PDSCH is being read. WAIT... \n");
        for (int i = 0; i < 25; i++) {
          if(fscanf(fd, "%lf", newPDSCH_RB+i));
        }
        fclose(fd);
      }
    }
  ...
  if (dlsch0 != NULL ) {
    ...
    for (rb=0; rb<frame_parms->N_RB_DL; rb++) {
     ...
     if (dlsch0) {
      ...
      #if (PDSCH_OKU == 1)
        //****** Change of Amplitudes ****** //
        /*
        Change the modulation symbols tables to be able to change
        the PDSCH symbols dynamically. 
        */
        switch (mod_order0) {
          case 2:
            qam_table_s0[0] *= newPDSCH_RB[rb];
            qam_table_s0[1] *= newPDSCH_RB[rb];
            break;
          case 4:
            qam_table_s0[0] *= newPDSCH_RB[rb];
            qam_table_s0[1] *= newPDSCH_RB[rb];
            qam_table_s0[2] *= newPDSCH_RB[rb];
            qam_table_s0[3] *= newPDSCH_RB[rb];
            break;
          case 6:
            qam_table_s0[0] *= newPDSCH_RB[rb];
            qam_table_s0[1] *= newPDSCH_RB[rb];
            qam_table_s0[2] *= newPDSCH_RB[rb];
            qam_table_s0[3] *= newPDSCH_RB[rb];
            qam_table_s0[4] *= newPDSCH_RB[rb];
            qam_table_s0[5] *= newPDSCH_RB[rb];
            break;
        ...
        switch (mod_order1) {
          case 2:
            qam_table_s1[0] *= newPDSCH_RB[rb];
            qam_table_s1[1] *= newPDSCH_RB[rb];
            break;
          case 4:
            qam_table_s1[0] *= newPDSCH_RB[rb];
            qam_table_s1[1] *= newPDSCH_RB[rb];
            qam_table_s1[2] *= newPDSCH_RB[rb];
            qam_table_s1[3] *= newPDSCH_RB[rb];
            break;
          case 6:
            qam_table_s1[0] *= newPDSCH_RB[rb];
            qam_table_s1[1] *= newPDSCH_RB[rb];
            qam_table_s1[2] *= newPDSCH_RB[rb];
            qam_table_s1[3] *= newPDSCH_RB[rb];
            qam_table_s1[4] *= newPDSCH_RB[rb];
            qam_table_s1[5] *= newPDSCH_RB[rb];
            break;
        ...
        allocate_REs(phy_vars_eNB,
                      ...
                      pilots,
                      ((pilots) ? (int16_t)(amp_rho_b * newPDSCH_RB[rb]) : (int16_t)(amp_rho_a * newPDSCH_RB[rb])),
                      get_pmi_temp,
                      qam_table_s0,
                      qam_table_s1,
                      &re_allocated,
